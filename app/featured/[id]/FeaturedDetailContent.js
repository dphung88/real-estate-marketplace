...
export default function FeaturedDetailContent({ item, images = [] }) {
  ...
  const isForest = titleLower.includes('forest') || titleLower.includes('mountain') || titleLower.includes('cabin') || titleLower.includes('park');
  const isLuxury = titleLower.includes('luxury') || titleLower.includes('villa');
  const isApartment = item.badge?.toLowerCase() === 'apartment' || titleLower.includes('apartment') || titleLower.includes('loft');
  ...
  return (
    <div className="detail-container">
      ...
      {/* Desktop Modal still available if needed, but not triggered by mobile thumbs anymore */}
      <ImageModal 
        images={safeImages} 
        isOpen={isModalOpen} 
        ...
        <div className="detail-main-content">
          ...
          <section id="overview" className="detail-section">
            <div className="detail-info-content">
              <h2 style={{ marginBottom: '8px', fontSize: '2.25rem', fontWeight: '700', color: 'var(--color-dark)' }}>{item.title}</h2>
              ...
              <div style={{ display: 'flex', gap: '32px', marginBottom: '28px', alignItems: 'baseline', flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '1.25rem', fontWeight: '600', color: '#555' }}>
                  {isApartment ? '2' : '4'} <span style={{ fontWeight: '400', fontSize: '1.1rem' }}>bd</span> | {isApartment ? '2' : '3'} <span style={{ fontWeight: '400', fontSize: '1.1rem' }}>ba</span> | {isApartment ? '1,200' : '2,500'} <span style={{ fontWeight: '400', fontSize: '1.1rem' }}>sqft</span>
                </span>
              </div>
          ...
          <section id="cost" className="detail-section">
            ...
            <div className="cost-calculator" style={{ ...beigeBgStyle, border: 'none' }}>
              <div className="calc-row">
                <span>Monthly {isApartment ? 'Rent' : 'Mortgage'}</span>
                <strong style={{ fontSize: '1.1rem' }}>{item.price}</strong>
              </div>
              <div className="calc-row">
              ...
              <div className="calc-row total" style={{ color: 'var(--color-dark)' }}>
                ...
                <span style={{ color: 'var(--color-accent)' }}>~ {item.price} + $195</span>
              </div>
            </div>
          </section>
        </div>
          ...
          <div style={{ ...beigeBgStyle, padding: '32px', boxShadow: '0 12px 40px rgba(27, 28, 54, 0.12)' }}>
            ...
            <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
              <p style={{ fontSize: '0.85rem', color: '#888', textAlign: 'center', marginBottom: '8px' }}>
                Property ID: <span style={{ fontWeight: '600', color: '#555' }}>{item.id}</span>
              </p>
